...
// Create a new CLICommander Instance
$cli = new CLICommander();

// Build the array of xterm colors
$colors = array(
...
// Loop through the colors and list their values
$cli->WriteLine("---------------------------------------------------------------");
$cli->WriteLine("| HTML Notation | Integer | Text Example | Background Example |");
$cli->WriteLine("---------------------------------------------------------------");
foreach ($colors as $html => $int) {
	$output = sprintf('| {||bold}%13s{reset} | {||bold}%7d{reset} | {'.$html.'}Text Example{reset} | {default|'.$html.'}                  {reset} |', $html, $int);
